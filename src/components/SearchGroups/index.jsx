...
const SearchGroups = () => {
    ...
    return (
        <Container>
            <InputPaper>
                <InputBase placeholder='Ex: "Menos fastfood!"' onChange={(e) => setSearch(e.target.value)}/>
                <SearchIcon style={{ color: cyan[900] }} />
            </InputPaper>
            <BackgroundDiv>
                <PaperDiv elevation='6'>
                    {filtered && filtered.map((item, index) => <Card key={index}><h3>{item.name}</h3><h5>{item.description}</h5><p>{item.category}</p> </Card>)}
                </PaperDiv>
            </BackgroundDiv>
        </Container>
    )
